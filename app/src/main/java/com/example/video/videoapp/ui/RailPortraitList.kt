...
@NonReusable
@Layout(R.layout.item_portrait_list)
class RailPortraitList (private val context : Context, private val imageList: List<Content>,
    ...
    fun onAttach(){
        ...
        for (i in 0 until  imageList.size){
            mPlaceHolderView.addView(RailPortrait(context, imageList[i].images.typeBanner,
                    imageList[i].title,listener, position))
        }
    }

    @Click(R.id.tv_more)
    fun onClick() {
        Log.e("RailPortraitList", "more clicked $position")
    }
}